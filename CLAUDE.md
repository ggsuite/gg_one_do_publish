...
**Remote pushes are retried on a dropped connection.** The bare-ref push of the merged default branch and the deletion of the remote feature branch run through gg_git's `GitRetry` (four attempts, 5 s / 15 s / 45 s apart) when stderr shows a transient transport failure (»closed by remote host«, »kex_exchange_identification«, »the remote end hung up unexpectedly«, a 5xx reply). A rejected push, missing credentials or an unknown repository fails at once, so the resume semantics below stay the safety net for everything else. The `ls-remote` lookup before the delete is not retried: its failure is already tolerated (the delete decides).
